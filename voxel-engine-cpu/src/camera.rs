...
impl Camera {
    ...
    pub fn arcball_rotate(&mut self, delta_pixels: Vec2, screen_size: (f32, f32)) {
        let mut position = Vec4::from((self.position, 1.0));
        let pivot = Vec4::from((self.target, 1.0));
        let right = self.view.transpose().col(0).xyz();

        let delta_angle_x = 2.0 * PI / screen_size.0;
        let delta_angle_y = 2.0 * PI / screen_size.0;

        let x_angle = delta_pixels.x * delta_angle_x;
        let mut y_angle = delta_pixels.y * delta_angle_y;
