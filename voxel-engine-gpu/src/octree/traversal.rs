...
use glam::{vec3, IVec2, UVec2, Vec3, Vec2, vec2, uvec2};
use spirv_std::num_traits::{Float, ToPrimitive};
    ...
    while scale < S_MAX {
        ...
        if (child_masks & 0x8000) != 0 && t_min <= t_max {
            ...
            if t_min <= tv_max {
                ...
                let mut ofs = node >> 17; // child ptr
                if (node & 0x10000) != 0 { // far
                    ofs = parent_idx + ofs * 2;
                }
                ofs += (child_masks & 0x7f).count_ones();
                parent_idx += ofs * 2;
